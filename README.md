...
```text
  ...
  讨论完成后，由 Codex 根据最终定稿修改/新建正式设定文档，并检查 git diff, 再提交本地仓库。
```

或者简化版本讨论提示词：
```text
$codex-pm-claude 使用“多次 Claude 进程 + 同一个临时讨论文件”的方式，与 Claude 做三轮讨论。任务结束后删除临时讨论文件。讨论主题：【这里写你的主题】，第一轮 claude 反驳 codex 初稿，第二轮 claude 二审 codex 修订，第三轮 claude 查漏。最后 codex 输出分歧、采纳点和最终定稿。检查 git diff, 再提交本地仓库。。
```
