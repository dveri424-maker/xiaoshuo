...
```text
 让 Codex 并行派发 4 个 worker 子任务：

  任务1：只处理 `新版正文/第001章-...` 到 `第005章-...`
  任务2：只处理 `新版正文/第006章-...` 到 `第010章-...`
  任务3：只处理 `新版正文/第011章-...` 到 `第015章-...`
  任务4：只处理 `新版正文/第016章-...` 到 `第020章-...`

  所有 worker 必须读取：
  - `.claude/skills/老嗷文风/SKILL.md`
  - `.claude/skills/活人感创作/SKILL.md`
  - `新版设定/` 下 README 指定的设定文档
  - 负责章节前后一章用于衔接

  要求：
  - 写作腔调贴主角意识流动，旁白贴韦德本人
  - 信息从感官、动作、误判、记忆触发和当场利害进入
  - 禁止出戏比喻、作者式总结、AI腔、剧情跳变、冷硬短句堆酷感
  - 每个 worker 只改自己负责章节，不改其他文件
  - 完成后列出改动文件和自检结果

  最后由 Codex 主进程统一检查章节交界、设定一致性、禁用表达、git diff，必要时兜底修正，并用中文提交本地仓库。
