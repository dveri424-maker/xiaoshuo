...
当前分支是中文恐怖片无限流幻想小说《恐怖片里的违规玩家》。

项目核心：林扬进入不同欧美恐怖类型副本，入场时不知道具体原型，只能通过现场线索、试错代价和怪物行为逐步识别规则；恐怖压迫成立后，他以“违规玩家”的方式审计死亡规则、抓出系统与怪物的条款漏洞，并用现场证据完成反向整改。
...
```text
使用 webnovel-workflow 与 meikong-horror-workflow，依据 CLAUDE.md、《恐怖片里的违规玩家》设定文档.md、《恐怖片里的违规玩家》剧情大纲与写法定稿.md，设计并写作第XXX章。

要求：
- 林扬入场或推进时不能全知，必须通过现场物件、人物反应、异常变化和试错代价缩小规则范围。
- 本章先成立恐怖压迫，再让林扬冷静拆解、标记违规点或反制。
- 至少出现一个可见物件、一个规则线索或代价、一个章末钩子。
- 林扬可以嘴欠，但必须服务观察、推理、压场、违规点识别或反制。
